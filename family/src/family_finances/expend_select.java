...
public class expend_select implements ActionListener,ItemListener{
	JFrame f7=new JFrame();
	Container ctp=f7.getContentPane();
	...
	JLabel lb5=new JLabel("支出金额:",JLabel.CENTER);
    ...
    String name[]= {"","编号查询","姓名查询","支出类型查询","支出时间查询","支出金额查询"};
					...
					b1.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent arg0) {
							...
							try {
								...
								{
									ResultSet rs1=null;
									rs1 = stmt.executeQuery("SELECT * FROM expend");
									while(rs1.next())        //如果rs1有数据，循环输出
									{
										String arr[]=new String[6];
										arr[0]=rs1.getString("eno");
										arr[1]=rs1.getString("name");
										arr[2]=rs1.getString("category");
										arr[3]=rs1.getString("expend_day");
										arr[4]=rs1.getString("expend_money");
									...
									ResultSet rs=null;
									rs = stmt.executeQuery("SELECT * FROM expend where eno='"+str1+"'");
								...
								if(c==2)
								{
									...
									if(str2.equals(null)||str2.equals(""))
									{
										...
										ResultSet rs=null;
										rs = stmt.executeQuery("SELECT * FROM expend where name='"+str2+"'");
									while(rs.next())        //如果rs有数据，循环输出
									{
										String arr[]=new String[6];
										arr[0]=rs.getString("eno");
								...
								if(c==3)
								{
									...
									if(str3.equals(null)||str3.equals(""))
									{
										...
										ResultSet rs=null;
										rs = stmt.executeQuery("SELECT * FROM expend where category='"+str3+"'");
									while(rs.next())        //如果rs有数据，循环输出
									{
										String arr[]=new String[6];
										arr[0]=rs.getString("eno");
								...
								if(c==4)
								{
									...
									if(str4.equals(null)||str4.equals(""))
									{
										...
										ResultSet rs=null;
										rs = stmt.executeQuery("SELECT * FROM expend where expend_day='"+str4+"'");
									while(rs.next())        //如果rs有数据，循环输出
									{
										String arr[]=new String[6];
										arr[0]=rs.getString("eno");
								...
								if(c==5)
								{
									...
									if(str5.equals(null)||str5.equals(""))
									{
										...
										ResultSet rs=null;
										rs = stmt.executeQuery("SELECT * FROM expend where expend_money='"+str5+"'");
									while(rs.next())        //如果rs有数据，循环输出
									{
										String arr[]=new String[6];
										arr[0]=rs.getString("eno");
							   ...
							   type=JOptionPane.PLAIN_MESSAGE;
								JOptionPane.showMessageDialog(f7, e2.getMessage(),title,type);
								}
					...
					b2.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent arg0) {
							...
							try {
								...
								if(c==1)
								{
								rs = stmt.executeQuery("SELECT * FROM expend where eno like '"+str1+""+"%"+"'");
								while(model.getRowCount()>0) {
									model.removeRow(model.getRowCount()-1);}//清空表格
								while(rs.next())        //如果rs有数据，循环输出
								{
								...
								if(c==2)
								{
								rs = stmt.executeQuery("SELECT * FROM expend where name like '"+str2+""+"%"+"'");
								while(model.getRowCount()>0) {
									model.removeRow(model.getRowCount()-1);}//清空表格
								while(rs.next())        //如果rs有数据，循环输出
								{
								...
								if(c==3)
								{
								rs = stmt.executeQuery("SELECT * FROM expend where category like '"+str3+""+"%"+"'");
								while(model.getRowCount()>0) {
									model.removeRow(model.getRowCount()-1);}//清空表格
								while(rs.next())        //如果rs有数据，循环输出
								{
								...
								if(c==4)
								{
								rs = stmt.executeQuery("SELECT * FROM expend where expend_day like '"+str4+""+"%"+"'");
								while(model.getRowCount()>0) {
									model.removeRow(model.getRowCount()-1);}//清空表格
								while(rs.next())        //如果rs有数据，循环输出
								{
								...
								if(c==5)
								{
								rs = stmt.executeQuery("SELECT * FROM expend where expend_money like '"+str5+""+"%"+"'");
								while(model.getRowCount()>0) {
									model.removeRow(model.getRowCount()-1);}//清空表格
								while(rs.next())        //如果rs有数据，循环输出
								{
